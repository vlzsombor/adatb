...
public interface OrszagRepository extends JpaRepository<Orszag, Long> {
    ...
    Orszag findOrszagByMegnevezes(String megnevezes);
...
//    List<String> elso_lekerdezes();
    ...
    @Query("SELECT new hu.szte.fenykepalbumok.utils.queryclass.OrszagDarab(o.megnevezes, count(f.id)) FROM Felhasznalo As f,Varos AS v ,Megye AS m, Orszag AS o where f.varos.id = v.id and v.megye.id = m.id and m.orszag.id= o.id group by o.megnevezes order by count(f.id) DESC ")
    ...
    @Query("SELECT new hu.szte.fenykepalbumok.utils.queryclass.OrszagDarab(ka.megnevezes, count(ke.id)) FROM Kep As ke, Kategoria AS ka where ke.kategoria.id=ka.id GROUP BY ka.megnevezes ORDER BY count(ke.id) DESC ")
  ...
  SELECT kepid, count(*) AS hozzaszolasok FROM kep, velemeny where kep.id = velemeny.kepid group by kepid;
     */
    @Query("SELECT new hu.szte.fenykepalbumok.utils.queryclass.OrszagDarab(k.id, count(v.id)) FROM Kep AS k, Velemeny AS v WHERE k.id=v.kep.id GROUP BY k.id ORDER BY k.id ASC ")
